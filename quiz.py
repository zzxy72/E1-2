...
class Quiz:  # Quiz를 “한 문제의 데이터 묶음”으로 명확하고 안전하게 표현하기 위해서 dataclass 사용.
    ...
    hint: str = ""  # hint에는 풀이 중 보여 줄 힌트 문장을 저장합니다.
    ...
    def __post_init__(self) -> None:  # 객체가 만들어진 직후 데이터가 올바른지 확인하는 메서드입니다.
        ...
        self.hint = str(self.hint).strip()  # 힌트도 문자열로 맞춘 뒤 공백을 정리합니다.
    ...
    def get_hint_text(self) -> str:  # 힌트가 없을 때도 화면에 보여 줄 문장을 반환하는 메서드입니다.
        if self.hint == "":  # 힌트가 비어 있으면 사용자에게 별도 안내를 보여 줍니다.
            return "힌트가 아직 등록되지 않았습니다."  # 빈 문자열 대신 안내 문장을 반환합니다.
        return self.hint  # 힌트가 있으면 그대로 반환합니다.
    ...
    def to_dict(self) -> dict[str, Any]:  # Quiz 객체를 JSON으로 저장하기 쉬운 딕셔너리 형태로 바꿉니다.
        return {  # 아래 중괄호 안에 저장할 데이터 구조를 만듭니다.
            ...
            "answer": self.answer,  
            "hint": self.hint,  # 힌트 문장도 함께 저장합니다.
        }  # 완성된 딕셔너리를 반환합니다.
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Quiz":  # 딕셔너리 데이터를 Quiz 객체로 바꾸는 메서드입니다.
        # " " " 전방 참조(Forward Reference)  -- 나중에 이 이름의 클래스가 나올 거야 라는 뜻입니다. 
        # Quiz 클래스가 아직 완전히 정의되기 전에 from_dict 메서드에서 Quiz 타입을 사용할 수 있게 해 줍니다.
        # isinstance()의 역할 :  객체(인스턴스)가 특정 클래스의 인스턴스인지, 혹은 그 클래스를 상속받은 자식 클래스의 인스턴스인지 확인하여 True 또는 False를 반환합니다.
        ...
        hint = str(data.get("hint", "")).strip()  # hint 키가 없을 수도 있으니 기본값을 빈 문자열로 둡니다.
        return cls(question=question, choices=choices, answer=answer, hint=hint)  # 정리된 값들로 새 Quiz 객체를 만들어 반환합니다.

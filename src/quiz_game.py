...
class QuizGame:  # QuizGame 클래스는 메뉴, 점수, 저장 상태를 관리합니다.
    ...
    def get_number_input(self, prompt_message: str, min_value: int, max_value: int) -> int | None:  # 일정 범위 안의 숫자 입력을 받을 때 사용하는 메서드입니다.
        while self.is_running:  # 게임이 실행 중인 동안에는 조건에 맞는 숫자가 들어올 때까지 반복합니다.
            text = self.get_non_empty_text(prompt_message)  # 먼저 빈 값이 아닌 문자열 입력을 공통 메서드로 받습니다.
            ...
            try:  # 문자열을 숫자로 바꾸는 과정에서 오류가 날 수 있어 try를 사용합니다.
                ...
                print(f"[주의] 숫자를 입력해 주세요. 가능한 범위는 {min_value}부터 {max_value}까지입니다.")  # 숫자 형식 오류를 쉽게 안내합니다.
                continue  # 올바른 숫자를 다시 입력받기 위해 반복문의 처음으로 돌아갑니다.
            if number < min_value or number > max_value:  # 숫자이긴 하지만 허용 범위를 벗어난 경우를 검사합니다.
                print(f"[주의] 잘못된 입력입니다. {min_value}-{max_value} 사이의 숫자를 입력하세요.")  # 범위 오류를 안내합니다.
    ...
    def show_menu(self) -> None:  # 메인 메뉴 화면을 출력하는 메서드입니다.
        print("\n========================================")  # 메뉴 상단 구분선을 출력합니다.
        print("         나만의 퀴즈 게임")  # 프로그램 제목을 출력합니다.
        print("========================================")  # 제목 아래 구분선을 출력합니다.
        print("1. 퀴즈 풀기")  # 첫 번째 메뉴 항목을 출력합니다.
        print("2. 퀴즈 추가")  # 두 번째 메뉴 항목을 출력합니다.
        print("3. 퀴즈 목록")  # 세 번째 메뉴 항목을 출력합니다.
        print("4. 점수 확인")  # 네 번째 메뉴 항목을 출력합니다.
        print("5. 종료")  # 다섯 번째 메뉴 항목을 출력합니다.
        print("========================================")  # 메뉴 하단 구분선을 출력합니다.

    def run(self) -> None:  # 프로그램 전체를 반복 실행하는 메인 루프 메서드입니다.
        print("\n[시작] 퀴즈 게임을 시작합니다.")  # 게임이 시작되었음을 사용자에게 알려 줍니다.
        while self.is_running:  # 종료를 선택하기 전까지 메뉴를 계속 보여 주기 위해 반복합니다.
            self.show_menu()  # 현재 사용할 수 있는 메뉴를 화면에 출력합니다.
            choice = self.get_number_input("선택: ", 1, 5)  # 메뉴 번호를 1부터 5 사이의 숫자로 입력받습니다.
            if choice is None:  # 입력 중단 등으로 메뉴 번호를 받지 못한 경우를 검사합니다.
    ...
    def handle_menu_choice(self, choice: int) -> None:  # 메뉴 번호에 따라 어떤 기능을 실행할지 결정하는 메서드입니다.
        if choice == 1:  # 1번을 고른 경우 퀴즈 풀기 기능을 실행합니다.
            self.play_quiz()  # 퀴즈를 실제로 푸는 메서드를 호출합니다.
        elif choice == 2:  # 2번을 고른 경우 퀴즈 추가 기능을 실행합니다.
            self.add_quiz()  # 새 퀴즈를 등록하는 메서드를 호출합니다.
        elif choice == 3:  # 3번을 고른 경우 퀴즈 목록 확인 기능을 실행합니다.
            self.list_quizzes()  # 현재 저장된 퀴즈 목록을 출력합니다.
        elif choice == 4:  # 4번을 고른 경우 점수 확인 기능을 실행합니다.
            self.show_best_score()  # 최고 점수와 플레이 횟수를 출력합니다.
        elif choice == 5:  # 5번을 고른 경우 종료 기능을 실행합니다.
    ...
    def play_quiz(self) -> None:  # 저장된 퀴즈를 순서대로 출제하는 메서드입니다.
        if not self.quizzes:  # 퀴즈가 하나도 없으면 게임을 진행할 수 없으므로 먼저 검사합니다.
            print("\n[주의] 등록된 퀴즈가 없어 퀴즈를 시작할 수 없습니다.")  # 퀴즈가 없음을 사용자에게 알려 줍니다.
            return  # 더 진행할 내용이 없으므로 메서드를 끝냅니다.
        print(f"\n[퀴즈] 퀴즈를 시작합니다. (총 {len(self.quizzes)}문제)")  # 몇 문제를 풀게 되는지 안내합니다.
        ...
        for index, quiz in enumerate(self.quizzes, start=1):  # 퀴즈 목록을 앞에서부터 하나씩 꺼내며 번호도 함께 만듭니다.
            print("\n----------------------------------------")  # 문제와 문제 사이를 보기 좋게 구분합니다.
            quiz.display(index)  # 현재 문제 번호와 문제 내용을 화면에 출력합니다.
            answer = self.get_number_input("정답 입력 (1-4): ", 1, 4)  # 사용자의 정답 번호를 1~4 사이로 입력받습니다.
            ...
            if quiz.is_correct(answer):  # 사용자의 답이 정답인지 확인합니다.
                ...
                print("[정답] 맞았습니다.")  # 정답 메시지를 출력합니다.
            else:  # 정답이 아닌 경우 이 블록이 실행됩니다.
                correct_text = quiz.get_correct_choice_text()  # 정답 번호에 해당하는 선택지 문장을 가져옵니다.
                print(f"[오답] 정답은 {quiz.answer}번: {correct_text}")  # 정답 번호와 내용을 함께 알려 줍니다.
        self.finish_round(score, len(self.quizzes))  # 모든 문제를 다 풀었다면 결과를 정리하고 저장합니다.

    def finish_round(self, score: int, total_questions: int) -> None:  # 한 번의 퀴즈 풀이가 끝났을 때 결과를 정리하는 메서드입니다.
        self.play_count += 1  # 퀴즈를 끝까지 한 번 완료했으므로 플레이 횟수를 1 늘립니다.
        print("\n========================================")  # 결과 화면 상단 구분선을 출력합니다.
        print(f"[결과] {total_questions}문제 중 {score}문제 정답")  # 몇 문제를 맞혔는지 결과를 출력합니다.
        print("========================================")  # 결과 화면 하단 구분선을 출력합니다.
        if score > self.best_score:  # 이번 점수가 기존 최고 점수보다 높은지 확인합니다.
            self.best_score = score  # 더 높은 점수라면 최고 점수를 새 값으로 갱신합니다.
            print("[기록] 새로운 최고 점수입니다.")  # 최고 점수 갱신 사실을 사용자에게 알려 줍니다.
        else:  # 기존 최고 점수를 넘지 못한 경우 이 블록이 실행됩니다.
            print(f"현재 최고 점수는 {self.best_score}점입니다.")  # 지금까지의 최고 점수를 다시 보여 줍니다.
        self.save_state()  # 바뀐 최고 점수와 플레이 횟수를 파일에 저장합니다.

    def add_quiz(self) -> None:  # 사용자가 새 퀴즈를 직접 입력해 추가하는 메서드입니다.
        print("\n[추가] 새로운 퀴즈를 추가합니다.")  # 퀴즈 추가 화면에 들어왔음을 알려 줍니다.
        question = self.get_non_empty_text("문제를 입력하세요: ")  # 비어 있지 않은 문제 문장을 입력받습니다.
        ...
        print("[완료] 퀴즈가 추가되었습니다.")  # 퀴즈 등록 성공 메시지를 출력합니다.

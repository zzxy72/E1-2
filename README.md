...
- 퀴즈 데이터, 최고 점수, 플레이 횟수, 게임 기록은 프로젝트 루트의 `state.json`에 저장됩니다.
...
`state.json`은 게임 상태를 저장하는 파일입니다. 관련 파일은 모두 프로젝트 루트에 위치합니다.
...
즉, `quiz_game.py` 파일 기준으로 프로젝트 루트의 `state.json`을 불러오고 저장하도록 구성했습니다.
